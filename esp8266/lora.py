...
import sx127x
#gc.collect()
u_id = (ubinascii.hexlify(machine.unique_id())).decode("utf-8")
...
def run():
    ...
    tr.setFrequency(config.lora_frequency, 000)  # kHz, Hz
    ...
    if MODE == 1:
        ...
        while True:
            tr.blink()
            print('begin transmitter-----====-----{}'.format(u_id))
            tr.send(u_id+"$"+"Hello,This is a test Data send program ,writen by Micropython based sx1278", FIXED)
            time.sleep_ms(1900)
