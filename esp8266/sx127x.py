...
class RADIO:
    ...
    def enableAFC(self, afc=True):
        ...
        if self._mode:
            ...
            if afc:
                reg |= 0x10  # bit 4: AfcAutoOn -> 1
            else:
                reg &= ~0x10  # bit 4: AfcAutoOn -> 0
    ...
    def setFixedLen(self, fixed=True):
        ...
        if self._mode:
            if self._fixedLen != fixed:  # set value only if different
                ...
                if fixed:
                    reg &= ~0x80  # bit 7: PacketFormat -> 0 (fixed size)
                else:
                    reg |= 0x80  # bit 7: PacketFormat -> 1 (variable size)
                self.writeReg(REG_PACKET_CONFIG_1, reg)
    ...
    def continuous(self, on=True):
        ...
        if self._mode:
            ...
            if on:
                reg &= ~0x40  # bit 6: `DataMode` 0 -> Continuous mode
            else:
                reg |= 0x40  # bit 6: `DataMode` 1 -> Packet mode
