...
class Serial:
    ...
    def __init__(self, uart_id=2, baudrate=115200, tx=17, rx=16, data_bits=8, stop_bits=1, parity=None, pins=None,ctrl_pin=None):
        # self._uart = UART(uart_id, baudrate=baudrate, bits=data_bits, parity=parity, \
        #                  stop=stop_bits, timeout_chars=10, pins=pins)
        self._uart = UART(uart_id, baudrate=baudrate, tx=tx, rx=rx, bits=data_bits, parity=parity, stop=stop_bits)
